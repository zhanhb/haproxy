...
smp_fetch_hdr_ip(const struct arg *args, struct sample *smp, const char *kw, void *private)
{
	...
	while ((ret = smp_fetch_hdr(args, smp, kw, private)) > 0) {
		if (smp->data.u.str.len < temp->size - 1) {
			...
			len = url2ipv4((char *) temp->str, &smp->data.u.ipv4);
			if (len > 0 && len == smp->data.u.str.len) {
				/* plain IPv4 address */
				smp->data.type = SMP_T_IPV4;
				break;
			} else if (len > 0 && temp->str[len] == ':' &&
				   strl2irc(temp->str + len + 1, smp->data.u.str.len - len - 1, &port) == 0 &&
				   port >= 0 && port <= 65535) {
