				  ...
				  const struct stream *strm, const char *pfx, uint32_t anon_key)
{
	...
	chunk_appendf(buf, "%s  scf=%p flags=0x%08x ioto=%s state=%s endp=%s,%p,0x%08x sub=%d", pfx,
		      ...
		      sdf ? sdf->se : NULL, sdf ? se_fl_get(sdf) : 0, scf->wait_event.events);
	...
	if (connf) {
		...
		chunk_appendf(buf,
		              ...
		              connf->flags,
		              conn_fd(connf),
		              conn_fd(connf) >= 0 ? fdtab[connf->handle.fd].state : 0,
		              conn_fd(connf) >= 0 ? !!(fdtab[connf->handle.fd].update_mask & ti->ltid_bit) : 0,
			      conn_fd(connf) >= 0 ? fdtab[connf->handle.fd].thread_mask: 0);
	}
	...
	chunk_appendf(buf, "%s  scb=%p flags=0x%08x ioto=%s state=%s endp=%s,%p,0x%08x sub=%d", pfx,
		      ...
		      sdb ? sdb->se : NULL, sdb ? se_fl_get(sdb) : 0, scb->wait_event.events);
	...
	if (connb) {
		...
		chunk_appendf(buf,
		              ...
		              connb->flags,
		              conn_fd(connb),
		              conn_fd(connb) >= 0 ? fdtab[connb->handle.fd].state : 0,
		              conn_fd(connb) >= 0 ? !!(fdtab[connb->handle.fd].update_mask & ti->ltid_bit) : 0,
			      conn_fd(connb) >= 0 ? fdtab[connb->handle.fd].thread_mask: 0);
	}
