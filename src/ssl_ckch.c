		...
		switch (appctx->st2) {
			...
			case SETCERT_ST_GEN:
				...
				list_for_each_entry_from(ckchi, &old_ckchs->ckch_inst, by_ckchs) {
					...
					appctx->ctx.ssl.err = NULL;
					if (new_ckchs->multi)
						errcode |= ckch_inst_new_load_multi_store(new_ckchs->path, new_ckchs, ckchi->bind_conf, ckchi->ssl_conf, sni_filter, fcount, &new_inst, &appctx->ctx.ssl.err);
					else
