...
struct dns_srvrq *new_dns_srvrq(struct server *srv, char *fqdn)
{
	...
	hostname_dn_len = dns_str_to_dn_label(fqdn, fqdn_len, trash.area,
					      trash.size);
...
struct dns_answer_item *find_srvrq_answer_record(const struct dns_requester *requester)
{
	...
	res = srv->srvrq->dns_requester->resolution;
				     ...
				     struct dns_resolution *resolution, int max_answer_records)
{
	...
	for (i = 0; i < dns_p->header.ancount; i++) {
		...
		if (found == 1) {
			...
			dns_answer_record->last_seen = now_ms;
			dns_answer_record->ar_item = NULL;
	...
	for (i = 0; i < dns_p->header.arcount; i++) {
		...
		found = 0;
		list_for_each_entry(tmp_record, &dns_p->answer_list, list) {
			struct dns_answer_item *ar_item;
