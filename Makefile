...
%.o:	%.c $(DEP)
	$(cmd_CC) $(COPTS) -MMD -MP -MT $@ -MF $*.d -c -o $@ $<
...
src/calltrace.o: src/calltrace.c $(DEP)
	$(cmd_CC) $(TRACE_COPTS) -MMD -MP -MT $@ -MF src/calltrace.d -c -o $@ $<
...
src/version.o:	src/version.c $(DEP)
	$(cmd_CC) $(COPTS) \
	      ...
	      -MMD -MP -MT $@ -MF src/version.d -c -o $@ $<
...
clean:
	$(Q)rm -f *.[oasd] src/*.[oasd] haproxy haterm test .build_opts .build_opts.new
